...
sleep 5 # wait for mysqld_safe to rev up, and check for port 3304
port_open=0

while [ "$port_open" -eq 0 ]; do
   /bin/nc -z -w 5 127.0.0.1 3304
   if [ $? -ne 0 ]; then
       echo "Sleeping waiting for port 3304 to open: result " $? 
       sleep 1
   else
       echo "Port 3304 is open"
       port_open=1
   fi
done
